    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.eval_freq == 0:
            ...
            if self.save_path is not None:
                ...
                plt.subplot(2, 1, 1)
                for i in range(self.eval_env.system.n_states):
                    plt.plot(np.arange(len(obss)), obss[:, i], label=labels[i])
                    plt.ylabel("Value")
                ...
                plt.subplot(2, 1, 2)
                for i in range(self.eval_env.system.n_states):
                    plt.plot(np.arange(len(norm_obss)), norm_obss[:, i], label=labels[i])
                    plt.ylabel("Value")
                    plt.xlabel(axis_name)
                    plt.xlim(axis[0], axis[-1])
                    plt.grid()
